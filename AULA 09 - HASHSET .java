import java.util.*;

class Cidade {
    String nomeCidade;
    String siglaEstado;

    public Cidade(String nomeCidade, String siglaEstado) {
        this.nomeCidade = nomeCidade;
        this.siglaEstado = siglaEstado;
    }

    public String getNomeCidade() {
        return nomeCidade;
    }

    public void setNomeCidade(String nomeCidade) {
        this.nomeCidade = nomeCidade;
    }

    public String getSiglaEstado() {
        return siglaEstado;
    }

    public void setSiglaEstado(String siglaEstado) {
        this.siglaEstado = siglaEstado;
    }

    @Override
    public String toString() {
        return "Cidade [nomeCidade=" + nomeCidade + ", siglaEstado=" + siglaEstado + "]";
    }

    @Override
    public int hashCode() {
        final int prime = 31;
        int result = 1;
        result = prime * result + ((nomeCidade == null) ? 0 : nomeCidade.hashCode());
        result = prime * result + ((siglaEstado == null) ? 0 : siglaEstado.hashCode());
        return result;
    }

    @Override
    public boolean equals(Object o) {
        if (o == null || getClass() != o.getClass()) return false;
        Cidade cidade = (Cidade) o;
        return Objects.equals(nomeCidade, cidade.nomeCidade) && Objects.equals(siglaEstado, cidade.siglaEstado);
    }
}

public class HashCidades {
    static void cadastrar(HashSet<Cidade> cidades) {
        String nome, estado;
        Scanner teclado = new Scanner(System.in);

        System.out.print("Nome cidade: ");
        nome = teclado.nextLine().toUpperCase();
        System.out.print("Estado cidade: ");
        estado = teclado.nextLine().toUpperCase();
        Cidade tmp = new Cidade(nome,estado);
        if (!cidades.add(tmp)) {
            System.out.println("Cidade neste estado já cadastrada");
        }

    }

    static void listar(HashSet<Cidade> cidades) {
        System.out.println("Listagem....");
        for (Cidade cidade : cidades) {
            System.out.println(cidade);
        }

    }

    static void pesquisar(HashSet<Cidade> cidades) {
        String nome, estado;
        Scanner teclado = new Scanner(System.in);

        System.out.print("Nome cidade: ");
        nome = teclado.nextLine().toUpperCase();
        List<Cidade> respostas = new ArrayList<>();

        for (Cidade cidade : cidades) {
            if (nome.equals(cidade.getNomeCidade())) {
                respostas.add(cidade);
            }
        }
        if (respostas.isEmpty()) {
            System.out.println("Cidade nao encontrada");
        } else {
            for (Cidade cidade : respostas) {
                System.out.println(cidade);
            }
        }
    }

    static void remover(HashSet<Cidade> cidades) {
        String nome, estado;
        Scanner teclado = new Scanner(System.in);

        System.out.print("Nome cidade: ");
        nome = teclado.nextLine().toUpperCase();
        System.out.print("Estado cidade: ");
        estado = teclado.nextLine().toUpperCase();
        Cidade tmp = new Cidade(nome,estado);
        if (cidades.remove(tmp))
            System.out.println("Cidade e estado removidos com sucesso");
        else
            System.out.println("Cidade neste estado nao localizada");

    }

    public static void main(String[] args) {
        HashSet<Cidade> cidades = new HashSet<>();

        char opcao;
        Scanner teclado = new Scanner(System.in);

        do {
            System.out.println("1 - Cadastrar");
            System.out.println("2 - Listar");
            System.out.println("3 - Pesquisar por cidade");
            System.out.println("4 - Remover por cidade");
            System.out.println("5 - Finalizar");
            System.out.print("Opcao: ");
            opcao = teclado.nextLine().charAt(0);

            switch (opcao) {
                case '1':
                    cadastrar(cidades);
                    break;
                case '2':
                    listar(cidades);
                    break;
                case '3':
                    pesquisar(cidades);
                    break;
                case '4':
                    remover(cidades);
                    break;
                case '5':
                    break;

                default:
                    System.out.println("Opcao invalida....");
                    break;
            }

        } while(opcao != '5');
    }


}
